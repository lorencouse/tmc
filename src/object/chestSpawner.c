...
void ChestSpawner_Type2Action4(ChestSpawnerEntity* this) {
    ...
    if ((super->frame & ANIM_DONE) != 0) {
        if (--super->subtimer == 0) {
            if (super->timer == 24) {
                ...
                if (sub_08084074(super->type2)) {
                    ...
                    /* Keep the reward available if the item-get pair could
                     * not be created. The player can retry after slots free. */
        ...
        /* Ordinary rewards commit in LinkHoldingItem after GiveItem. The
         * rupee fountain has no item-get entity and commits here instead. */
