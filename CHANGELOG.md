...
- Quicksaves capture every mutable game global, including entity lists,
  allocation counts, auxiliary entities and active item state (this fork's
  `gPortStateRegions`, which supersedes upstream's per-region relocation).
  A state from an earlier run resumes through the engine rather than being
  copied back. Normal `.sav` files are unaffected.
